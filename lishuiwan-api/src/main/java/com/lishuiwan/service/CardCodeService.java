...
public class CardCodeService {
    ...
    private final ConsumedMemberCodeMapper consumed;
    private final MembershipCardMapper cards;
    private final Jsons json;
    private final QrCodeService qrCodes;
    private final byte[] key;

    public CardCodeService(StringRedisTemplate redis, ConsumedMemberCodeMapper consumed, MembershipCardMapper cards, Jsons json, QrCodeService qrCodes, AppProperties p) {
        this.redis = redis;
        this.consumed = consumed;
        this.cards = cards;
        this.json = json;
        this.qrCodes = qrCodes;
        this.key = p.getJwt().getSecret().getBytes(StandardCharsets.UTF_8);
    }

    public Map<String, Object> issue(long memberId, long cardId) {
        MembershipCard card = cards.selectById(cardId);
        requireUsable(card, memberId);
        String nonce = Ids.nonce();
    ...
    public CodeData validate(String raw) {
        try {
            String token = raw.contains("token=") ? raw.substring(raw.indexOf("token=") + 6) : raw;
            ...
            long memberId = Long.parseLong(p.get("memberId").toString()), cardId = Long.parseLong(p.get("cardId").toString()), exp = Long.parseLong(p.get("exp").toString());
            String nonce = p.get("nonce").toString();
            if (Instant.now().getEpochSecond() >= exp || !((memberId + ":" + cardId).equals(redis.opsForValue().get("card-code:" + nonce))))
                throw invalid();
            if (consumed.selectCount(new LambdaQueryWrapper<ConsumedMemberCode>().eq(ConsumedMemberCode::getNonce, nonce)) > 0)
                throw invalid();
    ...
    public void consume(CodeData code) {
        ConsumedMemberCode c = new ConsumedMemberCode();
        c.setNonce(code.nonce());
        c.setMemberId(code.memberId());
        c.setAction("verification");
        c.setExpireAt(LocalDateTime.now().plusDays(1));
        consumed.insert(c);
    }

    public void clear(CodeData code) {
        redis.delete("card-code:" + code.nonce());
    }

    private void requireUsable(MembershipCard card, long memberId) {
        if (card == null || !card.getMemberId().equals(memberId)) throw BizException.notFound("权益卡");
        if (!card.getValidUntil().isAfter(LocalDateTime.now()) || !java.util.Set.of("unused", "using").contains(card.getStatus()))
            throw new BizException(42201, "该权益卡当前不可核销");
    }

    private BizException invalid() {
        return new BizException(41001, "权益卡核销码已过期或已使用", HttpStatus.GONE);
    }
