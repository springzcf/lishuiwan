...
public class VerificationService {
  private final MemberCodeService codes; private final MemberMapper members; private final MembershipCardMapper cards; private final VerificationMapper verifications;
  ...
  public VerificationService(MemberCodeService codes,MemberMapper members,MembershipCardMapper cards,VerificationMapper verifications,NotificationMapper notifications,IdempotencyService idem,Jsons json,AuthService auth,OrderItemMapper items){this.codes=codes;this.members=members;this.cards=cards;this.verifications=verifications;this.notifications=notifications;this.idem=idem;this.json=json;this.auth=auth;this.items=items;}
  public Map<String,Object> parse(String token){MemberCodeService.CodeData code=codes.validate(token);Member m=members.selectById(code.memberId());if(m==null||m.getStatus()!=0)throw BizException.notFound("会员");List<Map<String,Object>> available=cards.selectList(new LambdaQueryWrapper<MembershipCard>().eq(MembershipCard::getMemberId,m.getId()).in(MembershipCard::getStatus,"unused","using").orderByAsc(MembershipCard::getValidUntil)).stream().filter(c->c.getValidUntil().isAfter(LocalDateTime.now())).map(this::cardView).toList();return Map.of("member",auth.safe(m),"cards",available,"expiresAt",code.exp());}
  ...
  private Map<String,Object> verifyInternal(long operatorId,String requestNo,String token,long cardId,String benefitId,BigDecimal quantity,String operatorName){
    ...
    MemberCodeService.CodeData code=codes.validate(token);MembershipCard card=cards.selectForUpdate(cardId);
    ...
    if("unlimited".equals(type)){if(quantity.compareTo(BigDecimal.ONE)!=0)throw new BizException(42202,"不限次权益每次核销数量必须为 1");}
