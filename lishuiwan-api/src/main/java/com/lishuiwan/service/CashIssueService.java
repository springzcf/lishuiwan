...
public class CashIssueService {
  ...
  public Map<String,Object> issue(long operatorId,String requestNo,String token,long productId,BigDecimal paidAmount){return idem.execute("member",operatorId,"cash_issue",requestNo,Map.of("memberCodeToken",token,"productId",productId,"paidAmount",paidAmount),()->{MemberCodeService.CodeData code=codes.validate(token);Member member=requireActive(code.memberId());return orders.cashOnce(operatorId,member.getId(),productId,paidAmount);});}
