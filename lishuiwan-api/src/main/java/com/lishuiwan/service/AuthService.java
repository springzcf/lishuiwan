...
public class AuthService {
    ...
    public AuthService(WechatClient wechat, MemberMapper members, AdminUserMapper admins, TokenService tokens, PasswordEncoder passwords) {
        this.wechat = wechat;
    ...
    public Map<String, Object> wxLogin(String code) {
        String openid = wechat.openid(code);
        Member m = members.selectOne(new LambdaQueryWrapper<Member>().eq(Member::getOpenid, openid));
        if (m == null || m.getPhone() == null)
            return Map.of("registered", false, "registrationToken", tokens.registration(openid));
        ensureActive(m);
        return Map.of("registered", true, "token", tokens.member(m.getId()), "member", safe(m));
    }
    ...
    public Map<String, Object> bindPhone(String registrationToken, String phoneCode) {
        ...
        if (byOpenid != null) {
            ...
            members.updateById(byOpenid);
            m = byOpenid;
        } else if (byPhone != null) {
            if (!openid.equals(byPhone.getOpenid())) throw new BizException(40904, "手机号已绑定其他微信账号");
            m = byPhone;
        } else {
            m = new Member();
            m.setOpenid(openid);
