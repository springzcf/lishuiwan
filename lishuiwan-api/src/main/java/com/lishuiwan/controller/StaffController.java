...
public class StaffController {
  ...
  public record VerifyRequest(@NotBlank String requestNo,@NotBlank String memberCodeToken,@NotNull Long cardId,@NotBlank String benefitId,@NotNull @DecimalMin("0.01") BigDecimal quantity,@NotBlank @Size(max=50) String operatorName){}
  ...
  @PostMapping("/member-codes/parse") public ApiResponse<Map<String,Object>> parse(@Valid @RequestBody ParseRequest r){staff.require(RequestActor.memberId(),false);return ApiResponse.ok(verification.parse(r.token()));}
  @PostMapping("/verifications") public ApiResponse<Map<String,Object>> verify(@Valid @RequestBody VerifyRequest r){long id=RequestActor.memberId();staff.require(id,false);return ApiResponse.ok(verification.verify(id,r.requestNo(),r.memberCodeToken(),r.cardId(),r.benefitId(),r.quantity(),r.operatorName()));}
