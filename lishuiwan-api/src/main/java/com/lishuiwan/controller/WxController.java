...
public class WxController {
  private final CatalogService catalog; private final MemberMapper members; private final AuthService auth; private final MemberCodeService codes;
  ...
  public WxController(CatalogService catalog,MemberMapper members,AuthService auth,MemberCodeService codes,OrderService orders,VerificationService verification,VerificationMapper verifications,NotificationMapper notifications,WechatPayService wechatPay){this.catalog=catalog;this.members=members;this.auth=auth;this.codes=codes;this.orders=orders;this.verification=verification;this.verifications=verifications;this.notifications=notifications;this.wechatPay=wechatPay;}
  public record ProfileRequest(@Size(max=64) String nickname,@Size(max=255) String avatar){}
  ...
  @PutMapping("/member/profile") public ApiResponse<Map<String,Object>> update(@Valid @RequestBody ProfileRequest r){Member m=members.selectById(RequestActor.memberId());m.setNickname(r.nickname());m.setAvatar(r.avatar());members.updateById(m);return ApiResponse.ok(auth.safe(m));}
