...
Page({
  data:{products:[],productIndex:-1,paidAmount:'',member:null,memberCodeToken:'',adminPassword:'',scanning:false,submitting:false},
  async onLoad(){
  ...
  productChange(e){
    ...
    this.setData({productIndex,paidAmount:product.currentPrice,member:null,memberCodeToken:'',adminPassword:''})
  },
  amount(e){this.setData({paidAmount:e.detail.value})},
  password(e){this.setData({adminPassword:e.detail.value})},
  async scanMember(){
    ...
    try{
      ...
      this.setData({member:parsed.member,memberCodeToken:scan.result,adminPassword:''})
    }catch(error){
  ...
  async submit(){
    ...
    if(!this.data.adminPassword){wx.showToast({title:'请输入管理员密码',icon:'none'});return}
    ...
    try{
      await api.post('/wx/staff/cash-orders',{requestNo:api.uuid(),memberCodeToken:this.data.memberCodeToken,productId:product.id,paidAmount,adminPassword:this.data.adminPassword})
      wx.showToast({title:'发卡成功'})
      setTimeout(()=>wx.navigateBack(),1000)
    }finally{this.setData({submitting:false,adminPassword:''})}
  }
})
